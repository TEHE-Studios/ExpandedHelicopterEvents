...
function eHelicopter:crash()
	...
	if self.crashType then
		...
		if not returned_sq then
			...
			eventMarkerHandler.setOrUpdate(getRandomUUID(), "media/ui/crash.png", 900, heliX, heliY)
...
function eHelicopter:dropCarePackage(fuzz)
	...
	if returned_sq then
		...
		eventMarkerHandler.setOrUpdate(getRandomUUID(), "media/ui/airdrop.png", 900, heliX, heliY)
