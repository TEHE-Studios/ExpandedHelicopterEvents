...
function eHelicopter:initPos(targetedPlayer, randomEdge, initX, initY)
	...
	local offset = 500
	initX = initX or ZombRand(math.max(util.eheBounds.MIN_X, tpX-offset), math.min(util.eheBounds.MAX_X, tpX+offset)+1)
	initY = initY or ZombRand(math.max(util.eheBounds.MIN_Y, tpY-offset), math.min(util.eheBounds.MAX_Y, tpY+offset)+1)

	self.currentPosition = self.currentPosition or Vector3.new()

	if randomEdge then
		local initPosXY = {initX, initY}
		local minMax = {util.eheBounds.MIN_X, util.eheBounds.MIN_Y, util.eheBounds.MAX_X, util.eheBounds.MAX_Y}
	...
	--Looks for the closest edge to initX and initY to modify it to be along either util.eheBounds.MIN_X/Y/MAX_X/Y
	--differences between initX and util.eheBounds.MIN_X/Y/MAX_X/Y edge values
	local xDiffToMin = math.abs(initX-util.eheBounds.MIN_X)
	local xDiffToMax = math.abs(initX-util.eheBounds.MAX_X)
	local yDiffToMin = math.abs(initY-util.eheBounds.MIN_Y)
	local yDiffToMax = math.abs(initY-util.eheBounds.MAX_Y)
	--this list uses x/yDifftoMin/Max's values as keys storing their respective corresponding edges
	local xyDiffCorrespondingEdge = {[xDiffToMin]=util.eheBounds.MIN_X, [xDiffToMax]=util.eheBounds.MAX_X, [yDiffToMin]=util.eheBounds.MIN_Y, [yDiffToMax]=util.eheBounds.MAX_Y}
...
---@return number, number, number XYZ of eHelicopter
function eHelicopter:getXYZAsInt()
	if not self.currentPosition then return end

	local ehX = math.floor(util.Vector3GetX(self.currentPosition) + 0.5)
	local ehY = math.floor(util.Vector3GetY(self.currentPosition) + 0.5)
...
function eHelicopter:isInBounds()
	...
	if h_x and h_y and h_x < util.eheBounds.MAX_X+1 and h_x > util.eheBounds.MIN_X-1 and h_y < util.eheBounds.MAX_Y+1 and h_y > util.eheBounds.MIN_Y-1 then
		return true
	end
...
function eHelicopter:getDistanceToXY(x,y)
	...
	local a = x - util.Vector3GetX(self.currentPosition)
	local b = y - util.Vector3GetY(self.currentPosition)
...
function eHelicopter:getDistanceToVector(vector)
	...
	local x, y = util.Vector3GetX(vector), util.Vector3GetY(vector)
...
function eHelicopter:dampen(movement)
	...
	local x_movement = util.Vector3GetX(movement) * dampenFactor
	local y_movement = util.Vector3GetY(movement) * dampenFactor
...
function eHelicopter:aimAtTarget()
	...
	local movement_x = util.Vector3GetX(self.targetPosition) - util.Vector3GetX(self.currentPosition)
	local movement_y = util.Vector3GetY(self.targetPosition) - util.Vector3GetY(self.currentPosition)
...
function eHelicopter:move(re_aim, dampen)
	...
	local v_x = util.Vector3GetX(self.currentPosition)+(util.Vector3GetX(velocity)*timeSpeed)
	local v_y = util.Vector3GetY(self.currentPosition)+(util.Vector3GetY(velocity)*timeSpeed)
...
function eHelicopter:applyCrashChance(applyEnvironmentalCrashChance)
	...
	if applyEnvironmentalCrashChance then
		local _, weatherImpact = util.weatherImpact()
